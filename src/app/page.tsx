...
export default function HomePage() {
  ...
  const [monthlyRecs, setMonthlyRecs] = useState<
    ReturnType<typeof getMonthlyRecommendations>
  >([]);
    ...
    <div className="min-h-screen bg-[#FAF7F2] relative">
        ...
        <div className="max-w-2xl mx-auto px-4 h-14 flex items-center justify-between">
          <div className="flex items-center gap-2.5">
            <div
              ...
              style={{
                background:
                  "linear-gradient(to bottom right, #C8956C, #A67B5B)",
              }}
            >
              <svg
                viewBox="0 0 24 24"
            ...
            <span className="font-bold text-[#4A6670] text-lg tracking-wide">
              行囊
            </span>
          </div>
          <button
          ...
          >
            {showMenu ? (
              <X className="w-5 h-5 text-[#4A6670]/60" />
            ) : (
              <Menu className="w-5 h-5 text-[#4A6670]/60" />
            )}
          </button>
        </div>
      </nav>
            ...
            <div className="border-t border-[#C8956C]/10 pt-3">
              <div className="flex items-center gap-2 mb-3 px-3">
                ...
                <span className="text-xs text-[#4A6670]/40">
                  {mounted ? seasonLabels[currentSeason] : ""}
                </span>
              </div>
              <div className="grid grid-cols-3 gap-3 px-3">
                {monthlyRecs.slice(0, 3).map((city) => (
                  ...
                  >
                    <div
                      className="w-full aspect-[4/1] flex items-end p-2 relative"
                      style={getGradientStyle(city.gradient)}
                    >
                      <div className="absolute inset-0 bg-gradient-to-t from-black/20 to-transparent" />
                      <span className="text-white font-bold text-sm drop-shadow-md relative z-10">
                        {city.name}
                      </span>
                    </div>
                    <div className="w-full p-2.5 bg-white/60 backdrop-blur-sm">
                      <p className="text-xs text-[#4A6670]/70 line-clamp-1">
                        {city.duration}
                      </p>
                    </div>
                  </Link>
            ...
            <div className="border-t border-[#C8956C]/10 pt-3 space-y-1">
              ...
              >
                ...
                <div className="flex-1">
                  ...
                  <p className="text-xs text-[#4A6670]/40">
                    2小时可达的周末目的地
                  </p>
                </div>
              </Link>
      ...
      <main className="flex flex-col items-center justify-center min-h-[calc(100vh-7.5rem)] px-4">
        <div className="w-full max-w-md text-center">
          {/* 标题区 */}
          <h1 className="text-2xl font-bold text-[#4A6670] mb-2">
            把下一站，交给惊喜
          </h1>
          ...
          {blindboxPhase === "idle" && (
            <button
              ...
              style={{
                background:
                  "linear-gradient(135deg, #C8956C 0%, #A67B5B 50%, #8B6548 100%)",
              }}
            >
          ...
          {blindboxPhase === "shaking" && (
            <div className="w-full flex flex-col items-center justify-center py-8">
              <img
                src="/xingnang/manghe.gif"
                alt="盲盒开启"
                className="w-48 h-48 object-contain mb-4 animate-lc-float"
              />
              <p className="text-lg font-bold text-[#4A6670] animate-blindbox-shake">
                正在开启盲盒...
              </p>
            </div>
          )}
          ...
          >
            {hasFilters
              ? `已选 ${selectedDestTypes.length + selectedDuration.length + selectedBudget.length} 个偏好 · 展开`
              : "想限定范围？自定义偏好"}
          </button>
          ...
          {showFilters && (
            <div className="mt-4 lc-glass rounded-2xl p-4 animate-fade-in space-y-4 text-left">
              {/* 目的地类型 */}
              <div>
                <h3 className="text-xs font-semibold text-[#4A6670]/50 mb-2">
                  目的地类型
                </h3>
                ...
                <div className="flex flex-wrap gap-2">
                  {Object.entries(durationLabels).map(([key, label]) => (
                    <button
                      key={key}
                      onClick={() =>
                        toggleFilter(selectedDuration, setSelectedDuration, key)
                      }
                ...
                <div className="flex flex-wrap gap-2">
                  {Object.entries(budgetLabels).map(([key, label]) => (
                    <button
                      key={key}
                      onClick={() =>
                        toggleFilter(selectedBudget, setSelectedBudget, key)
                      }
                      className={`px-3 py-1.5 rounded-full text-xs transition-all ${
      ...
      <footer className="mt-8 pb-4 text-center">
        <p className="text-[10px] text-[#4A6670]/20">
          已收录 {allCities?.length * 2}+ 目的地与原创攻略
        </p>
      </footer>
    </div>
  );
