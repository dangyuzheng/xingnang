    ...
    <div className="min-h-screen bg-[#FAF7F2]">
      ...
      <main className="max-w-2xl mx-auto px-4 pt-6 pb-20">
        <div className="bg-gradient-to-br from-purple-100 to-indigo-50 rounded-2xl p-4 mb-6 flex items-center gap-3">
          ...
          <div>
            <h2 className="font-bold text-[#4A6670] text-sm">
              远离人潮，独享秘境
            </h2>
            <p className="text-xs text-[#4A6670]/50 mt-0.5">
              这些小众目的地大多数人是不知道的，信息零散的地方，这里最全面
            </p>
          </div>
        </div>
        ...
        <div className="space-y-3">
          {nicheCitiesList.map((city) => (
            ...
            >
              <div
                className={`w-16 h-16 rounded-xl bg-gradient-to-br ${city.gradient} flex items-center justify-center shrink-0`}
              >
              ...
              <div className="flex-1 min-w-0">
                ...
                <p className="text-xs text-[#4A6670]/60 mt-0.5">
                  {city.tagline}
                </p>
                <div className="flex flex-wrap gap-1 mt-1.5">
                  {city.destTypes.slice(0, 3).map((t) => (
                    <span
                      key={t}
                      className="flex items-center gap-0.5 text-[10px] px-1.5 py-0.5 rounded bg-[#F5EDE4] text-[#C8956C]"
                    >
                      {nicheIcons[t]}{" "}
                      {t === "mountain"
                        ? "雪山"
